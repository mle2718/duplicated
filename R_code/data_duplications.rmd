...
DERS_ALL_YEARS<-DERS_ALL_YEARS %>%
  relocate(any_of(c("YEAR", "MONTH", "DAY", "DEALNUM", "STATE_DNUM", "CF_LICENSE", "PERMIT", "HULLNUM", "SPPLNDLB", "SPPLIVLB", "SPPVALUE", "DERSOURCE", "LINK")))


# CFDBS.CFDERS2021
CURRENT.QUERY <- "select * from cfdbs.cfders2021 where nespp3=335 and year=2021 and month=1 and state=49 and cf_license='2147' order by day"
...
CAMS_DETS_AA<-CAMS_DETS_AA %>%
  relocate(any_of(c("CAMSID","YEAR", "MONTH", "DAY", "DEALNUM", "STATE_DNUM", "CF_LICENSE", "PERMIT", "HULLNUM", "SPPLNDLB", "SPPLIVLB", "SPPVALUE", "DERSOURCE", "LINK", "STATUS")))
...
DERS_AY<-DERS_ALL_YEARS %>%
  head(10) %>%
  select(any_of(c("YEAR", "MONTH", "DAY", "DEALNUM", "STATE_DNUM", "CF_LICENSE", "PERMIT", "HULLNUM", "SPPLNDLB", "SPPLIVLB", "SPPVALUE", "DERSOURCE", "LINK")))

kbl(DERS_AY, digits=0,booktabs=T, align=c("l",rep('r',times=12)), caption =  "Select data from CFDERS_ALL_YEARS") %>%
    kable_styling(full_width = T,latex_options = "hold_position") %>% 
      row_spec(0,bold=FALSE) 
...
The same problem occurs in the CFDERS_ALL_YEARS version of the table. CF_License and hullnum match,spplndlb and spplivlb sum up, and the date differs by 1 day.  The DERSOURCE is WHSE. 
...
```
Regular CFDERS is different.  Missing things are aggregated into day=0
